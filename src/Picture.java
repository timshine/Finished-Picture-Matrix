...
public class Picture extends SimplePicture {
	...
	public void grayScale() {
		...
		for (Pixel[] rowArray : pixels) {
			for (Pixel pixelObj : rowArray) {
				double average = pixelObj.getAverage(); // Finds average of colors
	...
	public void mirror() {
		Pixel[][] pixels = this.getPixels2D();
		//Sets values for horizontal mirror
	...
	public void upsideDown() {
		Pixel[][] pixels = this.getPixels2D();
		//Sets values for veritcal mirror
		int height = pixels.length;
		int width = pixels[0].length;
	...
	public void mirrorHorizontal() {
		...
		for (int col = 0; col < width; col++) {
			for (int row = 0; row < midIndex; row++) {
				top = pixels[row][col];
				bottom = pixels[lastIndex - row][col];
				bottom.setColor(top.getColor());  //Only switches top value
			}
		}
	}
	...
	public void mirrorDiagonal() {
		...
		//Changes diagonally
		for (int row = 0; row < height; row++) {
			for (int col = 0; col < height; col++) { //Must be height for square or out of bounds
				leftPixel = pixels[col][row];
				rightPixel = pixels[row][col];
				rightPixel.setColor(leftPixel.getColor()); //Left pixels dont change
			}
		}
	}
	...
	public void mirrorTemple() {
		...
		for (int row = 0; row < 100; row++) { //Row 100 is bottom of roof
			//vertical mirror flip the top
			for (int col = 0; col < midIndex; col++) {
				leftPixel = pixels[row][col];
				rightPixel = pixels[row][lastIndex - col];
				rightPixel.setColor(leftPixel.getColor());
